...
2. 建立資料庫，紀錄作品的發行社團，屬性標籤，聲優。各作品下載數每新增200次更新一次屬性標籤。
...
1. 編輯`DJVoiceConfig.pm.sample` 根據你的環境修改以下參數，並重新命名為`DJVoiceConfig.pm`：
...
2. 運行下列指令，會根據`DJVoiceConfig.pm` 定義的路徑修改各shell script。

    ```
    \./setup -p
    ```

3. 開始整理作品，將要建檔的作品檔案都放入目錄裡，假設在`/Users/hentai/myVoicePart1` 裡，然後運行下列指令，建議分
